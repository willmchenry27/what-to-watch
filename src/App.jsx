...
function topRatedSortScore(pick, currentWeekStr) {
  ...
  if (!pick.first_seen_week || !currentWeekStr) return pick.combined_score
...
function App() {
  ...
  // Blend cohorts:
  // - Top Rated bucket = simmered ∪ scored returning
  // - Fresh Drops bucket = fresh ∪ unscored returning
  const freshPicks = useMemo(
    () => allPicks.filter((p) =>
      p.cohort === 'fresh' || (p.cohort === 'returning' && p.combined_score == null)
    ),
    [allPicks]
  )
  const simmeredPicks = useMemo(
    () => allPicks.filter((p) =>
  ...
  // Fresh Drops — search, sort by popularity (blended fresh + unscored returning), reassign ranks
            ...
            <>
              ...
              {!hasToken ? (
                ...
                <div className="flex flex-col items-center justify-center py-20">
                  ...
                  {rawSavedPicks.length > 0 ? (
                    <p className="text-cream-300/50 text-sm">No saved picks match your search.</p>
                  ) : (
                    <>
